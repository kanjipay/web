...
import { AnalyticsEvent, AnalyticsManager, PageName } from "../../../utils/AnalyticsManager";
...
export default function BasketItem({ item, isEditing = false }) {
  ...
  const removeBasketItem = () => {
    removeItem(item)
    AnalyticsManager.main.logEvent(AnalyticsEvent.REMOVE_FROM_BASKET, { itemId: item.id, location: PageName.BASKET })
  }

  function changeBasketQuantity(increment) {
    const quantity = item.quantity + increment
    AnalyticsManager.main.logEvent(AnalyticsEvent.CHANGE_BASKET_AMOUNT, { itemId: item.id, quantity, location: PageName.BASKET })
