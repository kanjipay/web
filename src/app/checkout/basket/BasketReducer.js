...
function storeMerchant(merchant) {
  localStorage.setItem('basketMerchant', merchant ? JSON.stringify(merchant) : null);
}
...
export const sumItems = basketItems => {
  ...
  let total = basketItems.reduce((total, product) => total + product.price * product.quantity, 0)
...
export default function BasketReducer(state, action) {
  switch (action.type) {
    ...
    case BasketAction.CHANGE_QUANTITY:
      ...
      state.basketItems[state.basketItems.findIndex(item => item.id === itemId)].quantity = quantity
