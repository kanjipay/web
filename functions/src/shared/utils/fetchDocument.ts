...
export async function fetchDocument(collectionName: Collection, docId: string, checks: any = {}): Promise<any> {
  ...
  if (!doc.exists) {
    ...
    const error = new HttpError(HttpStatusCode.NOT_FOUND, errorMessage, errorMessage)
  ...
  for (const [fieldName, requiredValue] of checks) {
    ...
    if (actualValue !== requiredValue) {
      ...
      const error = new HttpError(HttpStatusCode.BAD_REQUEST, errorMessage, errorMessage)
      return generateErrorObject(error)
    }
  }
