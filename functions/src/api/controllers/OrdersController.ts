...
export default class OrdersController extends BaseController {
  ...
  create = async (req, res, next) => {
    ...
    if (nonexistantMenuItemTitles.length > 0) {
      ...
      const clientMessage = `We couldn't find the following item${isPlural ? "s" : ""} on the menu: ${nonexistantMenuItemTitles.join(", ")}. Please remove from your basket to continue.`
    ...
    if (unavailableMenuItemTitles.length > 0) {
      ...
      const clientMessage = `The following item${isPlural ? "s are" : " is"} currently unavailable: ${unavailableMenuItemTitles.join(", ")}. Please remove from your basket to continue.`
      next(new HttpError(HttpStatusCode.BAD_REQUEST, clientMessage))
      return 
    }
    ...
    for (const item of requested_items) {
      ...
      batch.set(
        db.doc(orderRef.path).collection("OrderItem").doc(),
        {
          order_id: orderId,
          menu_item_id: item.id,
          quantity: item.quantity,
