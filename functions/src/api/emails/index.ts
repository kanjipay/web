...
function constructTemplate(merchantName: string,orderNumber:number ,orderItems, total:number){
  console.log(orderItems);
  const orderItemClean = orderItems.map(o => ({quantity: o.quantity, price: o.price / 100, text:o.title }));
  console.log(orderItemClean);
  return {
    "receipt":true,
   "orderNumber":orderNumber,"merchantName":merchantName,
...
// Send email
function createMessage(toEmail: string, merchantName: string,orderNumber:number ,orderItems: Array<Object>, total:number){
  const templateData =constructTemplate(merchantName,orderNumber ,orderItems, total);
  return {
    to:toEmail,
    from: FROM_EMAIL,
...
async function sendEmail(toEmail: string, orderId: string) {
  const {merchantName,order_items,order_number,total} = await getOrderDetails(orderId);
  const emailMessage = await createMessage(toEmail,merchantName,order_number ,order_items, total)
