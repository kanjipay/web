...
describe("Navigate to product page", () => {
  it("Should navigate to product page", async () => {
    ...
    const firstListingTitle = await productListings[0].$(testName("product-listing-title"))
    expect(await getText(firstListingTitle)).to.eql("General Admission")
    const firstListingMessage = await productListings[0].$(testName("product-listing-message"))
    expect(await getText(firstListingMessage)).to.eql("£10.00")
    const secondListingMessage = await productListings[1].$(testName("product-listing-message"))
    ...
    const orderQuantityLabel = await page.$(testName("order-summary-line-item-quantity"))
