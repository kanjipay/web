...
describe("Get link", () => {
  ...
  it("Should succeed with valid link", (done) => {
    api.get(`/links/l/${validLinkId}`).end(function (err, res) {
  ...
  it("Should fail with expired link", (done) => {
    api.get(`/links/l/${expiredLinkId}`).end(function (err, res) {
  ...
  it("Should fail with used link", (done) => {
    api.get(`/links/l/${usedLinkId}`).end(function (err, res) {
      expect(res).to.have.status(400);
      done();
    });
  });
  it("Should fail with nonexistent link", (done) => {
    api.get(`/links/l/does-not-exist`).end(function (err, res) {
