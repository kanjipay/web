...
import CrezcoBankTile from "./CrezcoBankTile"
import { cancelPaymentIntent } from "./redirects"

export default function CrezcoChooseBankPage({ paymentIntent }) {
  ...
  useEffect(() => {
    axios.get(`${process.env.REACT_APP_BASE_SERVER_URL}/internal/api/v1/banks`).then(res => {
  ...
  useEffect(() => {
    if (bankCode && !isMobile) {
      ...
      createLink(`/checkout/${paymentIntent.id}/choose-bank?referringDeviceId=${deviceId}&bank=${bankCode}`).then(linkId => {
        setLinkId(linkId)
      })
    } else {
      setLinkId(null)
    ...
    return <div className="container">
      ...
      <div className="content">
        <div style={{ padding: "0 16px", textAlign: "center", margin: "auto", maxWidth: 311 }}>
          <CrezcoBankTile bankDatum={bankData.find(d => d.bankCode === bankCode)} style={{ width: 160, margin: "auto" }} />
          <Spacer y={2} />
      ...
      <div style={{ position: "absolute", bottom: 0, padding: "0 16px", textAlign: "center" }}>
        ...
        <p className="text-caption">
          By continuing you are permitting Crezco to initiate a payment from your bank account. You also agree to Moneyhub's
...

        {
          filteredBankData.length > 0 ?
            <div style={{ display: 'grid', gridTemplateColumns: "1fr 1fr", columnGap: 16, rowGap: 16 }}>
              {
                filteredBankData.map(datum => <CrezcoBankTile key={datum.bankCode} bankDatum={datum} onClick={() => setBankCode(datum.bankCode)} />)
              }
            </div> :
