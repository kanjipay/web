...
export default function GuestlistPage() {
  ...
  const handleSearchNameChange = event => {
    ...
    const filteredData = guestlistData.filter(datum => {
      ...
      return name.includes(newSearchName) || ticketId.replace("-", "").includes(newSearchName.replace("-", ""))
    })
  ...
  useEffect(() => {
    NetworkManager.get(`/merchants/m/${merchantId}/tickets/guestlist`, { eventId }).then(res => {
      ...
      <div className="content">
        ...
        <Spacer y={3} />
        {
          filteredGuestlistData.map((datum, index) => {
            const { productTitle, quantity, firstName, lastName, ticketId, earliestEntryAt, latestEntryAt } = datum

            const formatTimestamp = timestamp => format(dateFromTimestamp(timestamp), "do MMM H:mm")

            let entryTimeMessage = null

            if (earliestEntryAt) {
              if (latestEntryAt) {
                entryTimeMessage = `Entry from ${formatTimestamp(earliestEntryAt)} until ${formatTimestamp(latestEntryAt)}.`
              } else {
                entryTimeMessage = `Entry from ${formatTimestamp(earliestEntryAt)}.`
              }
            } else if (latestEntryAt) {
              entryTimeMessage = `Entry until ${formatTimestamp(latestEntryAt)}.`
            }

            const name = `${firstName} ${lastName}`
            const addition = quantity === 1 ? "" : ` +${quantity - 1}`
            return <div key={index}>
              <div style={{ backgroundColor: Colors.OFF_WHITE_LIGHT, padding: 16 }}>
                <h4 className="header-xs">{name}{addition}</h4>
                <Spacer y={2} />
                <p>{productTitle}</p>
                <Spacer y={1} />
