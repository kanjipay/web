  ...
  if (disabled) {
    ...
    >
      
      {
        close => <div>
          {
            allowsNull && <MenuItem title="Leave empty" onClick={() => handleSelectOption(null, close)} isSelected={!value} />
          }
          {
            optionList.map((option) => {
              return <MenuItem title={option.label ?? option.value} onClick={() => handleSelectOption(option.value, close)} isSelected={option.value === value} />
            })
          }
        </div>  
      }
