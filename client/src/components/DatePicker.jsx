...
export default function DatePicker({ 
  ...
  function valuesToDate(values) {
    ...
    for (const [name, valueAsString] of Object.entries(values)) {
      const valueAsInt = isNaN(valueAsString) ? 0 : name === "year" ? parseInt("20" + valueAsString, 10) : parseInt(valueAsString, 10)
      if (!isValid(name, valueAsInt)) { return null }
  ...
  function prependZeros(string, isFromInput = false) {
    if (isFromInput && string.replace("0", "") === "") { return string }
  ...
  function generateProps(name, placeholder, label = name[0].toUpperCase() + name.slice(1)) {
    return {
      name,
      label,
      style: inputStyle,
