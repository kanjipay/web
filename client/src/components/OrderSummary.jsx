  ...
  return <div>
    {
      lineItems.map(({ title, quantity, price }) => {
        return <div style={itemStyle}>
          ...
          <p test-name="order-summary-line-item-amount">{formatCurrency(price, currency)}</p>
        </div>
      })
    }
    
    {
      feePercentage && feePercentage > 0 && <div style={itemStyle}>
        <p>Processing fee</p>
        <div style={{ flexGrow: 100 }}></div>
        <p test-id="order-summary-processing-fee">{formatCurrency(fee, currency)}</p>
      </div>
    }
    ...
    <div style={itemStyle}>
      ...
      <div style={{ flexGrow: 100 }}></div>
      <p style={{ fontWeight: 500 }} test-id="order-summary-total">{formatCurrency(total, currency)}</p>
    </div>
  </div>
}